...
浏览器打开 `http://localhost:8020`，选择「专利」或「标准」模式。
...
```bash
# 单个文件
python extract_standards.py std.pdf --output result.json

# 批量处理
python extract_standards.py *.pdf --output result.json

# 处理整个目录
python extract_standards.py --dir ./standards --output result.json

# 自定义分块大小
python extract_standards.py std.pdf -c 200 -o result.json
```

### 方式四：日常启动服务

服务器为 WSL2 Ubuntu 环境，需通过 SSH 登录并配置端口转发。
...
uvicorn app.main:app --host 0.0.0.0 --port 18020
```

**本地端口转发 + 浏览器访问：**

新建一个 PowerShell 窗口（保持上面 ssh 窗口不关）：

```powershell
ssh -L 18020:localhost:18020 root@59.79.241.152
```

浏览器打开 `http://localhost:18020`。

**端口被占用**（address already in use）：

```bash
kill -9 $(lsof -t -i :18020)
uvicorn app.main:app --host 0.0.0.0 --port 18020
```
...
├── Dockerfile
├── docker-compose.yml
├── extract_standards.py        # 标准模式命令行入口
└── requirements.txt
```
