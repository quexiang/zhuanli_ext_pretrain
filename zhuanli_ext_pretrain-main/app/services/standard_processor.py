...
CONCRETE_CATEGORY_MAPPINGS: dict[str, str] = {
    "绿化形式": "绿化形式",
    "绿化类型": "绿化类型",
    "植物选择": "植物选择",
    "种植土": "种植土",
...
_USELESS_CATEGORIES: set[str] = set()
for v in CATEGORY_MAP.values():
    if v is not None:
        _USELESS_CATEGORIES.add(v)
...
def _is_useless_category(category: str) -> bool:
    return category in _USELESS_CATEGORIES


def _infer_category(title: str, chapter_number: str) -> str | None:
    """Infer category from chapter title and number. Returns None to filter."""
    title_clean = title.strip()

    if title_clean in _USELESS_CATEGORIES:
        return None

    if title_clean in CATEGORY_MAP:
        cat = CATEGORY_MAP[title_clean]
        if cat is None or _is_useless_category(cat):
            return None
        return cat

    for keyword, category in CATEGORY_MAP.items():
        if keyword in title_clean:
            cat = CATEGORY_MAP[keyword]
            if cat is not None and not _is_useless_category(cat):
                return cat
            return None

    if title_clean in CONCRETE_CATEGORY_MAPPINGS:
        return CONCRETE_CATEGORY_MAPPINGS[title_clean]
...
def _is_section_boundary(line: str) -> bool:
    ...
    if not stripped:
        return False
    # Section headers
    for header in CATEGORY_MAP:
        if header is not None and stripped == header:
            return True
    # Clause numbering
...
def _extract_chapters(text: str) -> list[dict]:
    ...
    if not chapter_boundaries:
        return [{"title": "全文", "content": text, "category": "技术规范",
                 "number": "0", "level": 1}]
...
def _process_chapters_parallel(
    ...
    def _process_single_chapter(chapter: dict) -> list[dict[str, str]]:
        ...
        if "范围" == title:
            if _has_substantive_content(content) and len(content) >= 30:
                return [{"text": content.strip(), "category": "范围"}]
            return []

        if "附录" in title:
            chunks = _chunk_text(content, min_size=30, max_size=chunk_size * 2)
            return [
                {"text": chunk, "category": "附录"}
                for chunk in chunks
        ...
        for part_title, part_content in appendix_parts:
            if "附录" in part_title:
                ...
                for chunk in chunks:
                    if len(chunk) >= 30 and _has_substantive_content(chunk):
                        records.append({"text": chunk, "category": "附录"})
                ...
                for chunk in chunks:
                    if len(chunk) >= min_chunk_len and _has_substantive_content(chunk):
                        if _noise_ratio(chunk) < 0.3:
                            records.append({"text": chunk, "category": category})
        return records
...
def _process_chapters_sequential(
    ...
    for chapter in chapters:
        ...
        if "术语和定义" in title or "术语" == title:
            ...
            for d in defs:
                if len(d) >= 20 and _has_substantive_content(d):
                    all_records.append({"text": d, "category": "术语和定义"})
            continue
        ...
        if "范围" == title:
            if _has_substantive_content(content) and len(content) >= 30:
                all_records.append({"text": content.strip(), "category": "范围"})
            continue
        ...
        if "附录" in title:
            ...
            for chunk in chunks:
                if len(chunk) >= 30 and _has_substantive_content(chunk):
                    all_records.append({"text": chunk, "category": "附录"})
            continue
        ...
        for part_title, part_content in appendix_parts:
            if "附录" in part_title:
                ...
                for chunk in chunks:
                    if len(chunk) >= min_chunk_len and _has_substantive_content(chunk):
                        if _noise_ratio(chunk) < 0.3:
                            all_records.append({"text": chunk, "category": category})
