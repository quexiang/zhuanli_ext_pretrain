...
def extract_text_direct(pdf_bytes: bytes) -> str:
    ...
    Walks every page, extracts text blocks, and joins them with form-feed
    page separators so downstream processing can detect page boundaries.

    Args:
        pdf_bytes: Raw PDF file bytes.

    Returns:
        Plain text from all pages, ``\\f``-separated, or empty string if
        no embedded text is found.
    """
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    parts: list[str] = []
