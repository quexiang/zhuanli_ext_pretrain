...
class OCREngine:
    ...
    def process_pdf(
        ...
        progress_callback: callable = None,   # 新增：可选回调
    ) -> str:
        """Run OCR on all pages of a PDF with progress logging.
        ...
        for i, img in enumerate(page_images):
            ...
            # 【核心改动】每 5 页打印一次进度，避免日志刷屏；最后一页也打
            if (i + 1) % 5 == 0 or (i + 1) == total:
                pct = (i + 1) / total * 100
                logger.info("⏳ OCR 进度: %d/%d 页 (%.1f%%)", i + 1, total, pct)
